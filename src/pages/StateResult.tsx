...
import {
  ...
  selectStatesCoverage,
  selectAspirantTotalsByStateLocal,
  selectAspirantTotalsFromStateByElection,
...
import { checkInState, listCollationPresence } from "../features/presence/presenceApi";
...
function StateCoverageBadge({ electionId }: { electionId: string }) {
  const coverage = useSelector(selectStatesCoverage(electionId));
  ...
  return (
    <span className="results-election-card__coverage">
      {coverage.entered} of {coverage.total} state{coverage.total !== 1 ? "s" : ""} ({pct}%) — {remaining} remaining
    </span>
  );
}
...
function ElectionLeadingBadge({
  ...
  collationResults?: StateCollationResult[];
}) {
...
function StateElectionCard({
  ...
  const stateTotalsSpecific = useSelector(
    selectAspirantTotalsByStateLocal(el._id, stateId)
  ) as AspirantTotalsShape | undefined;
  const stateTotalsAggregated = useSelector(
    selectAspirantTotalsFromStateByElection(el._id)
  ) as AspirantTotalsShape | undefined;
  // State-specific if available, otherwise fall back to election-wide aggregated state totals
  const stateTotals = (stateTotalsSpecific?.aspirants?.length ? stateTotalsSpecific : stateTotalsAggregated) as AspirantTotalsShape | undefined;

  const collationData = useSelector(selectStateCollation(el._id, stateId));
  const collationResults: StateCollationResult[] = collationData?.results ?? [];

  const coverage = useSelector(selectStatesCoverage(el._id));
  const aspirantStateWins = coverage?.aspirantStateWins ?? [];
  ...
  const totalFromAggregated = (stateTotals?.aspirants ?? []).reduce((sum, a) => sum + (a.totalVotes ?? 0), 0);
  const totalSubmitted = totalFromCollation > 0 ? totalFromCollation : totalFromAggregated;

  // Leading aspirant ID
  const leadingAspirantId = (() => {
    const totals = stateTotals?.aspirants ?? [];
  ...
  const leadingStateWins = leadingAspirantId
    ? (aspirantStateWins.find((w) => String(w.aspirantId) === String(leadingAspirantId))?.statesWon ?? null)
    : null;
    ...
    <div className="results-election-card">
        ...
        <div className="results-election-card__header-main">
          <div className="results-election-card__header-top">
            ...
            <span className="results-election-card__header-coverage">
              <StateCoverageBadge electionId={el._id} />
            </span>
          </div>
          <div className="results-election-card__header-meta results-election-card__header-meta--spaced">
            <ElectionLeadingBadge aspirantTotals={stateTotals} collationResults={collationResults} />
            {totalSubmitted > 0 && (
              <span className="results-election-card__total-votes results-election-card__total-votes--right">
                Total state votes: {totalSubmitted.toLocaleString()}
                {leadingStateWins !== null && (
                  <span className="results-election-card__ward-wins">
                    &nbsp;· Winning {leadingStateWins} state{leadingStateWins !== 1 ? "s" : ""}
                  </span>
                )}
              </span>
            )}
      ...
      {isExpanded && (
        <div className="results-election-card__body">
          {isStateAgent && !hasMarkedPresenceResolved ? (
            <div className="results-pu-card results-pu-card--mark-presence" style={{ maxWidth: "20rem" }}>
              ...
              <div className="results-pu-card__body">
                <span className="results-pu-card__label">State presence only</span>
            ...
            <>
              <div className="results-aspirants-list">
                {expandedAspirants.map((a) => {
                  const totalsEntry = stateTotals?.aspirants?.find(
                  ...
                  // Use state-specific collation votes if available, otherwise show aggregated total votes
                  const submittedVotes = collationEntry?.votes ?? totalsEntry?.totalVotes ?? null;
                  const partyCode = (
                    a.partyCode ?? a.party?.acronym ?? collationEntry?.aspirant?.partyCode ?? totalsEntry?.aspirant?.partyCode ?? ""
                  ).toUpperCase();
...
const STATE_AGENT_ROLE = "state_constituency_returning_officer_agent";
...
export default function StateResult() {
  ...
  const u = user as { state?: LocationRef } | null;
  const stateId = getLocationId(u?.state);
  const stateName = getLocationName(u?.state);
  ...
  useEffect(() => {
    if (expandedElectionId) {
      ...
      if (stateId) {
        dispatch(getResultsByElectionAndState({ electionId: expandedElectionId, stateId }));
      }
    }
  }, [dispatch, expandedElectionId, stateId]);

  useEffect(() => {
    if (electionsList.length > 0) {
      electionsList.forEach((el) => {
        dispatch(getStatesCoverage({ electionId: el._id }));
        if (OVERVIEW_ROLES.includes(role) || role === STATE_AGENT_ROLE) {
          dispatch(getAspirantTotalsFromStateByElection({ electionId: el._id }));
        }
        if (stateId) {
          dispatch(getAspirantTotalsByElectionAndState({ electionId: el._id, stateId }));
          dispatch(getResultsByElectionAndState({ electionId: el._id, stateId }));
          if (role === STATE_AGENT_ROLE) {
            dispatch(listCollationPresence({ electionId: el._id, stateId }));
          }
        }
      });
    }
  }, [dispatch, electionsList.map((e) => e._id).join(","), stateId, role]);

  const handleAccordionToggle = (id: string) => {
    setExpandedElectionId((prev) => (prev === id ? null : id));
    setAccordionMessage(null);
    dispatch(getStatesCoverage({ electionId: id }));
    if (OVERVIEW_ROLES.includes(role) || role === STATE_AGENT_ROLE) {
      dispatch(getAspirantTotalsFromStateByElection({ electionId: id }));
    }
    if (id && stateId) {
      dispatch(getAspirantTotalsByElectionAndState({ electionId: id, stateId }));
      dispatch(getResultsByElectionAndState({ electionId: id, stateId }));
      if (role === STATE_AGENT_ROLE) {
        dispatch(listCollationPresence({ electionId: id, stateId }));
      }
    }
    if (id) {
  ...
  const handleMarkPresence = async (electionId: string) => {
    ...
    setAccordionMessage(null);
    try {
      await dispatch(checkInState({ stateId, electionId })).unwrap();
      setAccordionMessage({ type: "success", text: "State presence marked. You can now enter results." });
      dispatch(listCollationPresence({ electionId, stateId }));
    } catch (err: unknown) {
      const msg =
  ...
  const handleSaveAspirantResult = async (electionId: string, aspirantId: string) => {
    if (!stateId) {
      setAccordionMessage({
        type: "error",
        text: "Your account is not assigned to a state.",
      });
      return;
    }
    ...
    try {
      ...
      setAccordionMessage({ type: "success", text: "State result saved successfully." });
      dispatch(getStatesCoverage({ electionId }));
      if (OVERVIEW_ROLES.includes(role) || role === STATE_AGENT_ROLE) {
        dispatch(getAspirantTotalsFromStateByElection({ electionId }));
      }
      dispatch(getAspirantTotalsByElectionAndState({ electionId, stateId }));
      dispatch(getResultsByElectionAndState({ electionId, stateId }));
      setAspirantVotes((prev) => ({
        ...prev,
        [electionId]: {
          ...(prev[electionId] ?? {}),
    ...
    if (ok > 0) {
      setAccordionMessage({
        ...
        text: errMsg
          ...
          : `All ${ok} state result(s) saved successfully.`,
      });
      dispatch(getStatesCoverage({ electionId }));
      if (OVERVIEW_ROLES.includes(role) || role === STATE_AGENT_ROLE) {
        dispatch(getAspirantTotalsFromStateByElection({ electionId }));
  ...
  if (role !== STATE_AGENT_ROLE && !isOverviewRole) {
    return (
      <div className="results-page">
        <div>
          <p className="results-page__breadcrumb">EMS / State Results</p>
          <h1 className="results-page__title">State Results</h1>
        </div>
        <div className="results-card" style={{ marginTop: "1rem" }}>
          <p className="results-view__empty">
            This page is for State Returning Officers only. You need the{" "}
            <strong>state_constituency_returning_officer_agent</strong> role to enter state results and votes.
          </p>
        </div>
      </div>
    );
    ...
    <div className="results-page">
      ...
      {electionsList.length > 0 && (
        <div className="results-card results-elections-section">
          <h2 className="results-card__heading">State presence only</h2>
          <p className="results-elections-section__hint">
            {role === STATE_AGENT_ROLE
              ? "Mark your presence for each election to enter state results."
          ...
          <div className="results-elections-grid">
            {electionsList.map((el) => (
              <StateElectionCard
                ...
                isStateAgent={role === STATE_AGENT_ROLE}
      ...
      {role === STATE_AGENT_ROLE && showMessagesModal && (
      ...
      {role === STATE_AGENT_ROLE && showReportModal && (
        <ResultsReportModal
          isOpen={showReportModal}
          onClose={() => setShowReportModal(false)}
          electionsList={electionsList}
          locationBody={stateId ? { stateId } : {}}
        />
      )}
    </div>
  );
